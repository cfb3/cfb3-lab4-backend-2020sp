...
/**
 * @api {get} /marvel/characters Request a single random marvel character
 * @apiName GetMarvelCharacters
 * @apiGroup Marvel
 * 
 * @apiHeader {String} authorization JWT provided from Auth get
 ...
 * @apiDescription This end point is a pass through to the marvel.com api/ 
 */ 
router.get("/characters/random", (request, response) => {
    ...
    let id = Math.floor(Math.random() * 1493);

    let params = `?ts=${time_stamp}&apikey=${public_key}&hash=${hash}&offset=${id}&limit=1`
 ...
 * @apiDescription This end point is a pass through to the marvel.com api/ 
 */ 
router.get("/characters/search/:name", (request, response) => {
    ...
    let params = `?ts=${time_stamp}&apikey=${public_key}&hash=${hash}&nameStartsWith=${name}`
